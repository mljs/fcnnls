...
import { fcnnlsVector } from '../fcnnlsVector.ts';

import { assertResult } from './assertResult.ts';
import { data } from './data/convergence.ts';
import { data as data1 } from './deconv-examples/single_shifted.ts';
...
test('negative identity X and positive RHS', () => {
...
test('simple case that requires a negative coefficient', () => {
  const X = new Matrix([
    [1, 0],
    [2, 0],
    [3, 0],
...
test('non-singular square X, Y 3x1', () => {
  const X = new Matrix([
    [0, 1, 1],
    [1, 0, 1],
    [1, 1, 0],
...
test('singular square X rank 2, Y 3x1', () => {
  const X = new Matrix([
    [1, 2, 3],
    [4, 5, 6],
    [7, 8, 9],
