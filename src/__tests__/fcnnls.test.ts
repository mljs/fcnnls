...
import { assertResult } from './assertResult.ts';

const concentration = readFileSync(
  join(import.meta.dirname, 'data/matrix.txt'),
  'utf8',
);
...
const matrix = new Matrix(A).transpose();

const proportion = readFileSync(
  join(import.meta.dirname, 'data/x_fcnnls.txt'),
  'utf8',
);
...
const answer = new Matrix(k.toSpliced(133, 1));

const observation = readFileSync(
  join(import.meta.dirname, 'data/target.txt'),
  'utf8',
);
...
const target = new Matrix(b).transpose();

test('identity X, Y 5x3', () => {
...
test('Van Benthem - Keenan example', () => {
  const X = new Matrix([
    [95, 89, 82],
    [23, 76, 44],
    [61, 46, 62],
...
test('matrix/target should throw here', () => {
  const X = matrix;
  const Y = target;
...
test('matrix/target with maxIterations exactly the same as it should be', () => {
  const X = matrix;
  const Y = target;
...
test('debuggage random matrices', () => {
  const X = new Matrix([
    [269, 134, 116, 940, 530],
    [899, 199, 207, 95, 533],
...
test('identity X, negative Y 3x1', () => {
